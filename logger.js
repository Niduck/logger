...
export default class Logger {
  static snack = {
    append: function (log, element, color, ttl) {
      let _log = log;
    ...
    remove: function (id) {
      let [_containerId, _id] = id.split("--");
      let container = document.getElementById(_containerId);
      let child = container.getElementsByClassName(`logsnack--${_id}`)[0];
      if (child) {
        container.removeChild(child);
        container.style.removeProperty(`--logsnack-tag-color-${_id}`);
        container.style.removeProperty(`--logsnack-tag-background-${_id}`);
      }
    },
  };
}
